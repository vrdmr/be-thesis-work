...
public abstract class GAStringsSeq extends GA {
	 ...
	 * @throws GAException
	 */
	public GAStringsSeq(int chromosomeDim, int populationDim,
			double crossoverProb, int randomSelectionChance,
			int maxGenerations, int numPrelimRuns, int maxPrelimGenerations,
			double mutationProb, int chromDecPts, String[] possGeneValues,
			int crossoverType, boolean computeStatistics) throws GAException {
	...
	/**
	 * Randomly reassign a gene in the chromosome identified by the given index
	 * (iChromIndex) to one of the possible gene values
	 * 
	 * @param iChromIndex
	 */
	...
	protected void initPopulation() {
		for (int i = 0; i < populationDim; i++) {
			for (int j = 0; j < chromosomeDim; j++)
				((ChromStrings) this.chromosomes[i]).setGene(
						getRandomGeneFromPossGenes(), j);
	...
	protected void doTwoPtCrossover(Chromosome Chrom1, Chromosome Chrom2) {
		...
		if (iCrossoverPoint2 == (iCrossoverPoint1 + 1))
			...
			String gene1_Chrom1 = ((ChromStrings) Chrom1)
					.getGene(iCrossoverPoint1);
			String gene1_Chrom2 = ((ChromStrings) Chrom2)
					.getGene(iCrossoverPoint1);
			String gene2_Chrom1 = ((ChromStrings) Chrom1)
					.getGene(iCrossoverPoint2);
			String gene2_Chrom2 = ((ChromStrings) Chrom2)
					.getGene(iCrossoverPoint2);

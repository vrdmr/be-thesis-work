...
public abstract class GA implements Runnable {
	/**
	 * probability of a mutation occuring during genetic mating. For example,
	 * 0.03 means 3% chance
	 */
	double mutationProb;
	...
	/**
	 * prelim generations. Prelim runs are useful for building fitter "starting"
	 * chromosome stock before the main evolution run.
	 */
	...
	/**
	 * number of chromosomes to evolve. A larger population dim will result in a
	 * better evolution but will slow the process down
	 */
	...
	abstract protected void doOnePtCrossover(Chromosome Chrom1,
			Chromosome Chrom2);

	/** do two point crossover between the two given chromosomes */
	abstract protected void doTwoPtCrossover(Chromosome Chrom1,
			Chromosome Chrom2);

	/** do uniform crossover between the two given chromosomes */
	abstract protected void doUniformCrossover(Chromosome Chrom1,
			Chromosome Chrom2);
	...
	/*
	 * Varad: We will have to think about x and y chromosomes in order to get a
	 * solution of the cluster we are trying to get ..
	 */
	...
	public GA(int chromosomeDim, int populationDim, double crossoverProb,
		...
		/*
		 * Assigning the values to internal data structures
		 */
	}
	...
	/**
	 * Gets the number of preliminary runs that will be performed before the
	...
	/**
	 * Main routine that runs the evolution simulation for this population of
	 * chromosomes.
	 * 
	...
	public int evolve() {
		...
		if (numPrelimRuns > 0) {
			...
			for (int iPrelimRuns = 1; iPrelimRuns <= numPrelimRuns; iPrelimRuns++) {
				...
				// create a somewhat fit chromosome population for this prelim
				// run
				while (iGen < maxPrelimGenerations) {

					/*
					 * System.out.println(iPrelimRuns + " of " + numPrelimRuns +
					 * " prelim runs --> " + (iGen + 1) + " of " +
					 * maxPrelimGenerations + " generations");
					 */
				...
				// copy these somewhat fit chromosomes to the main chromosome
				// pool
				int iNumPrelimSaved = 0;
				for (int i = 0; i < populationDim
						&& iNumPrelimSaved < iPrelimChromToUsePerRun; i++)
					if (this.chromosomes[i].fitnessRank >= populationDim
							- iPrelimChromToUsePerRun) {
						this.prelimChrom[iPrelimChrom + iNumPrelimSaved]
		...
		// VaradMeru: if no prelim runs were conducted then ...

		else
			System.out.println("INITIAL POPULATION (NO PRELIM RUNS):");

		// Add Preliminary Chromosomes to list box
		addChromosomesToLog(0, 20);

		/*
		 * VaradMeru: Initial Population now displayed thnx to 389
		 */

		iGen = 0;
		/*
		 * VaradMeru: while ( all the generations are computed) all the
		 * computations are carried in this while loop
		 */
		while (iGen < maxGenerations) {
			computeFitnessRankings();// fittest and the worst chromosome found
			// out here
			doGeneticMating(); // random selection and crossover
			copyNextGenToThisGen(); // mutatution done inside it
		...
		System.out.println("GEN " + (iGen + 1) + " AVG FITNESS = "
				+ this.genAvgFitness[iGen - 1] + " AVG DEV = "
				+ this.genAvgDeviation[iGen - 1]);

		addChromosomesToLog(iGen, 100); // display Chromosomes to system.out

		computeFitnessRankings(); // after so many generations
		...
		System.out.println("Chromosome Number: " + this.bestFitnessChromIndex);
	...
	/**
	 * Go through all chromosomes and calculate the average fitness (of this
	 * generation)
	 * 
	 * @return double
	 */
	...
	public void selectTwoParents(int[] indexParents) {
		...
		// while a parent isn't found
		while (bFound == false) {
			index = getRandom(populationDim); // get random member of population

			/*
			 * Varad: In our case it is 5% :BinaryOnes
			 * 
			 * It can be thought of as a question is 5 gr8r than getRandon(100)
			 * and how many times 5 will be greater than it ??? only 5 % times
			 * thus your random selection technique done
			 */
			if (randomSelectionChance > getRandom(100)) {
				indexParent1 = index;
				bFound = true; // got a random parent
			} else {
				// the greater a chromosome's fitness rank, the higher prob that
				// it itself will be selected to reproduce
				// +1 to include self
		...
		while (bFound == false) {
			...
			if (randomSelectionChance > getRandom(100)) {
				...
				if ((index != indexParent1)
					...
					// if
					// (this.chromosomes[index].getNumGenesInCommon(this.chromosomes[indexParent1])+1
					// > getRandom(chromosomeDim))
					// {
					// indexParent2 = index;
					// bFound = true;
					// }
	 ...
	 * fitness is lower than any fitness value for any chromosome in the current
	 * generation, the fitnessRank will equal zero.
	 * 
	 * @param fitness
	 * @return int the fitness ranking
	 */
	/*
	 * VaradMeru: This method describes the methodology below
	 * 
	 * I have a better fitness than how many of the other chromos in the
	 * population thus fitnessRank is the number of chromosomes your fitness is
	 * higher than
	 */
	...
	void computeFitnessRankings() {
		...
		// VaradMeru: get the initial fitness of every one in the population

		for (int i = 0; i < populationDim; i++)
			this.chromosomes[i].fitnessRank = getFitnessRank(this.chromosomes[i].fitness);
		// VaradMeru: get the fitnessRank from 527

		double rBestFitnessVal;
		double rWorstFitnessVal;
		// -1 to exclude self
		// best and worst fitnessRank stored
		for (int i = 0; i < populationDim; i++) {
	 ...
	 * fittest 2 chromosomes always survive to the next generation). This way an
	 * extremely fit chromosome is never lost from our chromosome pool.
	 */
	void doGeneticMating() {
		int iCnt, iRandom;
		int indexParent1 = -1, indexParent2 = -1;
		Chromosome Chrom1, Chrom2; // Temp
		iCnt = 0;

		/*
		 * VaradMeru: Elitism--fittest chromosome automatically go on to next
		 * gen (in 2 offspring)
		 */

		/*
		 * VaradMeru: A key observation in the output is that the first two
		 * chromosomes are always identical This is due to the fact that elitism
		 * is employed. The Fittest chromosomes are copied to the first two
		 * chromosomes of the next generation
		 */
		this.chromNextGen[iCnt]
				.copyChromGenes(this.chromosomes[this.bestFitnessChromIndex]);
		iCnt++;
		this.chromNextGen[iCnt]
				.copyChromGenes(this.chromosomes[this.bestFitnessChromIndex]);
		iCnt++;

		// VaradMeru: initializing the tempChromes
		...
		do {
			...
			/*
			 * VaradMeru: Look for copyChromGenes(...) in ChromeChars.java
			 */
			...
			{
				// all the PtXovers written in GAFloat/String/...
				if (this.crossoverType == Crossover.ctOnePoint)
					...
					// VaradMeru: Randomizing the choice of crossovers ...
					iRandom = getRandom(3);
					if (iRandom < 1)
						doOnePtCrossover(Chrom1, Chrom2);
					else if (iRandom < 2)
			...
			// if no crossover, then copy this parent chromosome "as is" into
			// the offspring
			{
		...
		// VaradMeru: While the next generation's population is equal to the
	...
	void copyNextGenToThisGen() {
		for (int i = 0; i < populationDim; i++) {
			...
			if (i != this.bestFitnessChromIndex) {
				...
				if ((i == this.worstFitnessChromIndex)
						|| (getRandom(1.0) < mutationProb))
					doRandomMutation(i);
			}
		}
	}
	 ...
	 */
	/*
	 * VaradMeru: The method to show the chromosomes on screen
	 * 
	 * VaradMeru: If the number of chromosome to display is greater than the
	 * number of chromosomes in the population --> only display the number of
	 * chromosome equal to number of genes in each chromosome
	 */

	void addChromosomesToLog(int iGeneration, int iNumChromosomesToDisplay) {

		/*
		 * VaradMeru: eg if there are 10 chromosomes to be displayed and number
		 * of chromosomes in the population are 30 then no need to get in the if
		 * stmt . But the problem occurs when we are dealing with 40 chromosomes
		 * and the number of chromosomes in the population is 10 then kicks in
		 * the condition below. So iNumChromosomesToDisplay =this.chromosomeDim
		 * means the number of chromosomes to be displayed is then equal to its
		 * dimension which is equal to number of genes it has
		 */
		...
		for (int i = 0; i < iNumChromosomesToDisplay; i++) {
			this.chromosomes[i].fitness = getFitness(i);
			/*
			 * VaradMeru: fitness is defined in the example
			 */
			sGen = "" + iGeneration;
			if (sGen.length() < 2)
				sGen = sGen + " ";
			// if the number is a single-digit number the
			// add a space between itself and ':'
			sChrom = "" + i;
			if (sChrom.length() < 2)
				sChrom = sChrom + " ";// if the number is a single-digit number
			// the add a space between itself and :
	...
	/**
	 * Get the average deviation from the current population of chromosomes. The
	 * smaller this deviation, the higher the convergence is to a particular
	 * (but not necessarily optimal) solution. It calculates this deviation by
	 * determining how many genes in the populuation are different than the
	 * bestFitGenes. The more genes which are "different", the higher the
	 * deviation.
	 * 
	 * @return
	 */
	/*
	 * VaradMer: How many of the chromosomes are not the best or equal to it?
	 * The avgDeviation counts the number of chromosomes which are not equal to
	 * the best chromosome ..
	 */
	protected double getAvgDeviationAmongChroms() {
		...
		for (int iGene = 0; iGene < this.chromosomeDim; iGene++) {
			if (this instanceof GAString) {
				char bestFitGene = ((ChromChars) this.chromosomes[this.bestFitnessChromIndex])
						.getGene(iGene);
				for (int i = 0; i < this.populationDim; i++) {
					char thisGene = ((ChromChars) this.chromosomes[i])
							.getGene(iGene);
					if (thisGene != bestFitGene)
						devCnt++; // deviation count
				}
			} else if (this instanceof GAFloat) {
				double bestFitGene = ((ChromFloat) this.chromosomes[this.bestFitnessChromIndex])
						.getGene(iGene);
				for (int i = 0; i < populationDim; i++) {
					double thisGene = ((ChromFloat) this.chromosomes[i])
							.getGene(iGene);
					if (thisGene != bestFitGene)
						devCnt++;
				}
			...
			{
				String bestFitGene = ((ChromStrings) this.chromosomes[this.bestFitnessChromIndex])
						.getGene(iGene);
				for (int i = 0; i < this.populationDim; i++) {
					String thisGene = ((ChromStrings) this.chromosomes[i])
							.getGene(iGene);
	...
	/**
	 * Take a binary string and convert it to the long integer. For example,
	 * '1101' --> 13
	 * 
	 * @param sBinary
	 * @return long

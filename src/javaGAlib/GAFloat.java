...
public abstract class GAFloat extends GA {
	/**
	 * the number of decimal points precision to use. If the chromosome has a
	 * dimension of 10 and the decPtsPrecision = 9, numbers will be stored in
	 * the chromosomes like "0.123456789"
	 */
	...
	/**
	 * Create random chromosomes from the given gene space (floating pt
	 * numbers).
	 */
		...
		for (int i = 0; i < populationDim; i++) {
			for (int iGene = 0; iGene < chromosomeDim; iGene++) {
				if ((this.positiveNumOnly == true) || (getRandom(100) > 50))
					((ChromFloat) this.chromosomes[i]).genes[iGene] = getRandom(1000.0)
							/ (1 + getRandom(1000.0));
				else
					((ChromFloat) this.chromosomes[i]).genes[iGene] = -getRandom(1000.0)
							/ (1 + getRandom(1000.0));
	 ...
	 * @throws GAException
	 */
	public GAFloat(int chromosomeDim, int populationDim, double crossoverProb,
			...
			int decPtsPrecision, boolean positiveNumOnly,
		...
		if (decPtsPrecision < 0)
			throw new GAException(
					"decPtsPrecision must be zero (for integers) or greater. It cannot be negative.");
		if (chromosomeDim < 1)

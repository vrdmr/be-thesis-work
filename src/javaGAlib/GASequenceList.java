...
public abstract class GASequenceList extends GAString {
	...
	/**
	 * Take the given chromosome which may have duplicated genes and eliminate
	 * any duplicated genes by replacing duplicates with genes which were left
	 * out of the chromosome (that is they exist in the gene space but not in
	 * the given chromosome). For example if gene space = 'ABCDEFGH' and sChrom1
	 * = 'ABCDDEFG', then the gene 'D' is duplicated and 'H' has been left out.
	 * Swap the 2nd 'D' with 'H' and the chromosome is fixed.
	 * 
	 * @param sChromosome
	...
	protected String getChromWithoutDuplicates(String sChromosome) {
		...
		// first get a string (which acts as a list) of all genes left OUT of
		// this chrom
		sGenesLeftOut = "";
		...
		for (int i = 0; i < chromosomeDim; i++) {
			...
			if (iPos > -1) // gene also found in a later part of the chromosome,
							// it is duplicated!
			{
				// assign this duplicate gene a random value from the list of
				// genes left out
				iRandomGeneLeftOut = getRandom(sbGenesLeftOut.length());
				sbChromosome.setCharAt(iPos + i + 1, sbGenesLeftOut
						.charAt(iRandomGeneLeftOut));

				// now take this "gene left out" out of the list (string) of
				// available genes
		...
		for (int i = 0; i < populationDim; i++) {
			...
			for (int iGene = 0; iGene < chromosomeDim; iGene++) {
				...
				do {
					sGene = ""
							+ this.possGeneValues
									.charAt(+getRandom(this.possGeneValues
											.length()));
	...
	protected void doOnePtCrossover(Chromosome Chrom1, Chromosome Chrom2) {
		...
		// Now eliminate any duplicated genes by replacing duplicates with genes
		// which
		// were left out of the chromosome.
		// For example if gene space = 'ABCDEFGH' and sChrom1 = 'ABCDDEFG', then
		// the
		// gene 'D' is duplicated and 'H' has been left out. Swap the 2nd 'D'
		// with 'H' and
	 ...
	 * @throws GAException
	 */
	public GASequenceList(int pChromosomeDim, int pPopulationDim,
			double pCrossoverProb, int pRandomSelectionChance,
			int pMaxGenerations, int pNumPrelimRuns, int pMaxPrelimGenerations,
			double pMutationProb, int pChromDecPts, String pPossGeneValues,
			int pCrossoverType, boolean computeStatistics) throws GAException {
		super(pChromosomeDim, pPopulationDim, pCrossoverProb,
				pRandomSelectionChance, pMaxGenerations, pNumPrelimRuns,
				pMaxPrelimGenerations, pMutationProb, pChromDecPts,
				pPossGeneValues, pCrossoverType, computeStatistics);
